...
def simu(p):
    ...
    parameters = {
        ...
        "mesh_parameters": {
            ...
            "size": size,
            "refinements": [{"cells": 300, "x": 3e-6}, {"cells": 50, "x": 30e-9}],
        },
        "boundary_conditions": [
            {"surface": [1, 2], "value": 0, "component": 0, "type": "dc"}
        ],
        "temperature": {
            "type": "expression",
            "value": 300
        ...
        "exports": {
            "derived_quantities": {
                ...
                "average_volume": [{"field": "T", "volumes": [1]}],
                "total_volume": [
                    {"field": "retention", "volumes": [1]},
                    {"field": "solute", "volumes": [1]},
                    {"field": "1", "volumes": [1]},
                    {"field": "2", "volumes": [1]},
                ],
            }
...
def error(p):
    ...
    print(
        "Average absolute error is :" + str(err) + " " + str(fatol) + " " + str(xatol)
    )
    ...
    def minimise_with_neldermead(ftol, xtol, x):
        ...
        while goon:
            ...
            if a == "no" or a == "No":
                ...
                if c != "":
                    new_xatol = float(c)
                minimise_with_neldermead(
                    new_fatol, new_xatol, np.array([res.x[0], res.x[1]])
                )
